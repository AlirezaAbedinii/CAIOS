...
- [x] Curated medical catalogue live (`catalog/`) — 46 modules down to 9, in a
      fork; plus 12 curated bioimage.io models on the AI4Life loader, defaulting
      to connectomics
- [x] CAIOS logo, palette, favicon, title strings — generated by
      `scripts/make-brand-assets.py`; the dashboard had been serving *no* logo
      and *no* favicon at all
- [x] Every `cloud.ai4eosc.eu` link repointed or removed — the runtime config is
      clean and the analytics beacon is disabled. Two addresses remain compiled
      into the JS bundle as fallbacks, overridden at start; `check-branding.sh`
      reports them as warnings rather than pretending they are gone
- [x] `docs/demo-script.md` written and timed — 22 minutes over 7 beats

**Gate: mechanically passed 2026-08-16.** `scripts/check-branding.sh` verifies
the checkable half — no foreign branding, no third-party tracking, artwork that
is really artwork, a catalogue matching `catalog/keep.txt` where every module
deploys.

The other half is a judgement and is deliberately left to a person: open the
dashboard and ask someone who does not know the project what they think it is
